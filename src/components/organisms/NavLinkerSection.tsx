  ...
  return (
    <div className={clsx('flex flex-col items-center gap-8', 'md:flex-row')}>
      <NavLinker
        ...
        onClick={() => navigateTo('/insight/today')}
      />
      ...
      <NavLinker
        label="수확물 창고"
        isActive={pathname === '/insight/storage'}
        onClick={() => navigateTo('/insight/storage')}
      />
      ...
      {isLogin && (
        <div className="md:hidden">
          <NavLinker
            label="마이페이지"
            isActive={pathname === '/profile/my'}
            onClick={() => navigateTo('/profile/my')}
          />
        </div>
      )}
    </div>

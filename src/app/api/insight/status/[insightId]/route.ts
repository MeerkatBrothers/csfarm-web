...
import insightStatusSource from '@/features/insight/datasources/insightStatusSource';
import {
  InsightStatusResDto,
  insightStatusResDtoSchema,
} from '@/features/insight/dtos/response/insightStatusResDto';
...
export const GET = async (
  _: Request,
  context: Params,
): Promise<NextResponse<Result<InsightStatusResDto>>> => {
  try {
    ...
    const apiResponse: ApiResponse<InsightStatusResDto> = await insightStatusSource(
      insightId,
      storedAccessToken,
    );

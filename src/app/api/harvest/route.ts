...
import harvestInsightSource from '@/features/insight/apis/server/harvestInsightSource';
import {
  HarvestInsightReqDto,
  harvestInsightReqDtoSchema,
} from '@/features/insight/dtos/request/harvestInsightReqDto';

export const POST = async (request: Request): Promise<NextResponse<Result<null>>> => {
  try {
    const requestBody: unknown = await request.json();
    const validatedRequestBody: HarvestInsightReqDto = validateOrThrow(
      harvestInsightReqDtoSchema,
      requestBody,
    );

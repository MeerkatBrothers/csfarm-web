...
import harvestedInsightSource from '@/features/insight/apis/server/harvestedInsightSource';
import {
  HarvestedInsightResDto,
  harvestedInsightResDtoSchema,
} from '@/features/insight/dtos/response/harvestedInsightResDto';

export const GET = async (
  request: Request,
): Promise<NextResponse<Result<HarvestedInsightResDto>>> => {
  try {
    ...
    const apiResponse: ApiResponse<HarvestedInsightResDto> = await harvestedInsightSource(
      page,
      size,
      storedAccessToken,
    );
    ...
    const validatedData: HarvestedInsightResDto = validateOrThrow(
      harvestedInsightResDtoSchema,
      data,
    );

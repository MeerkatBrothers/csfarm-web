...
const insightDetailSource = async (
  insightId: number,
): Promise<ApiResponse<InsightDetailResDto>> => {
  const endpoint: string = `insight/detail/${insightId}`;

  const apiResponse: ApiResponse<InsightDetailResDto> = await externalFetcher<
    ApiResponse<InsightDetailResDto>
  >({

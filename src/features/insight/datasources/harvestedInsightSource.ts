...
const harvestedInsightSource = async (
  page: number,
  size: number,
  accessToken: string,
): Promise<ApiResponse<HarvestedInsightResDto>> => {
  const endpoint: string = `insight/harvested?page=${page}&size=${size}`;

  const apiResponse: ApiResponse<HarvestedInsightResDto> = await externalFetcher<
    ApiResponse<HarvestedInsightResDto>
  >({

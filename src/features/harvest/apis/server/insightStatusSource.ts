...
const insightStatusSource = async (
  insightId: number,
  accessToken: string,
): Promise<ApiResponse<InsightStatusResDto>> => {
  const endpoint: string = `insight/status/${insightId}`;

  const apiResponse: ApiResponse<InsightStatusResDto> = await externalFetcher<
    ApiResponse<InsightStatusResDto>
  >({

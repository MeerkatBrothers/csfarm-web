...
const quizStatusSource = async (
  quizId: number,
  accessToken: string,
): Promise<ApiResponse<QuizStatusResDto>> => {
  const endpoint: string = `quiz/status/${quizId}`;

  const apiResponse: ApiResponse<QuizStatusResDto> = await externalFetcher<
    ApiResponse<QuizStatusResDto>
  >({
